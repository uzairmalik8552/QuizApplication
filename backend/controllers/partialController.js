...
const partialSearch = async (req, res) => {
  try {
    ...
    console.log("2");
    console.log(results);
    // this is theoptimized code as the above one takes time to go through the etire data and then return the response
    const hasNextPage = await UserSearch.findOne(query)
      .select("_id")
      .skip(page * limit)
      .lean()
      .then((doc) => !!doc);
